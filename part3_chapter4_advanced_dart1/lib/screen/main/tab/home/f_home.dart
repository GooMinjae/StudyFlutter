...
class _HomeFragmentState extends State<HomeFragment> {
  ...
  Future<void> veryHeavyComputationWorkWithIsolateSpawn() async {
    ...
    final isolate = await Isolate.spawn((port) {
      // Isolate
      // animation: main Isolate
      // spawn -> count -> 내부적으로 실행
      // 별도의 공간에서 수행 -> 기존 화면의 상태, 전역변수 공유 불가
      // -> progressListenPort.sendPort 로 해결 가능
      // -> {} -- map 형태로 데이터 받을 수 있음
      int count = 0;
      ...
      debugPrint(
        ...
        onExit: exitPort
            .sendPort); // progressListenPort.sendPort, onError: errorPort.sendPort, onExit: exitPort.sendPort
